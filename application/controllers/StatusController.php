...
class StatusController extends Controller
{
    ...
    public function postAction(){
        ...
        $user = $this->session->get('user');
        $status_obj = $this->db_manager->get('Status');

        if (count($errors) === 0){
            $status_obj->insert($user['id'], $body);
            return $this->redirect('/');
        }
        $statuses = $status_obj->fetchAllPersonalArchivesByUserId($user['id']);
        return $this->render(array(
            'errors'   => $errors,
            'body'     => $body,
            'statuses' => $statuses,
    ...
    public function userAction($params){
        ...
        return $this->render(array(
            ...
            'statuses'  => $statuses,
            'following' => $following,
            '_token'    => $this->generateCsrfToken('account/follw'),
        ));
    }
